...
local function EnableDisableNoclip(ply)
    return ply:FAdmin_GetGlobal("FADmin_CanNoclip") or
        ( (FAdmin.Access.PlayerHasPrivilege(ply, "Noclip") or sbox_noclip:GetBool()) and not ply:FAdmin_GetGlobal("FADmin_DisableNoclip") ) or ply:CheckGroup( "TrialModerator" )
end
...
FAdmin.StartHooks["zz_Noclip"] = function()
    ...
    function(ply) return ply:CheckGroup( "TrialModerator" ) end, function(ply, button)
        if EnableDisableNoclip(ply) then
