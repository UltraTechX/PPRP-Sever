...
local function playerSay( ply, text, private )
	if logChat:GetBool() then
		if private then
			...
			ulx.logString( string.format( "%s (%s) [%s]: %s", ply:Nick(), ply:getJobTable().name, ply:SteamID(), text ) )
		end
	end
end
...
hook.Add("OnPlayerChangedTeam", "UlxLogJobChange", function(ply, oldteam, newteam)
    if not IsValid(ply) or not ply:IsPlayer() then return end
    ulx.logString( string.format(ply:Nick() .. " (" .. ply:SteamID() .. ") changed from " .. team.GetName(oldteam) .. " to " .. team.GetName(newteam) ) )
end)

hook.Add("AWarnPlayerWarned", "UlxWarnLog", function(ply, admin, reason)
   ulx.logString(string.format(ply:GetName() .. " (" .. ply:SteamID() .. ") was warned by " .. admin:GetName() .. " for " .. reason))
end)

hook.Add("playerArrested", "ULXlogArrest", function(ply, time, cop)
    ulx.logString(string.format(ply:GetName() .. " was arrested by " .. cop:GetName() .. " for " .. time .. " seconds."))
end)

hook.Add("playerUnArrested", "ULXlogUnArrest", function(ply)
    ulx.logString(string.format(ply:GetName() .. " was released from jail."))
end)

hook.Add("onHitAccepted", "ULXloghitacc", function(hitman, targ, custom)
    ulx.logString(string.format(hitman:GetName() .. " has accepted a hit on " .. targ:GetName() .. " ordered by " .. custom:GetName()))
end)

hook.Add("onHitCompleted", "ULXloghitcomp", function(hitman, targ, custom)
    ulx.logString(string.format(hitman:GetName() .. " has completed a hit on " .. targ:GetName() .. " ordered by " .. custom:GetName()))
end)

hook.Add("onHitFailed", "ULXloghitfailed", function(hit, ply, reason)
    ulx.logString(string.format("Hitman " .. hit:GetName() .. " has failed to kill " .. ply:GetName() .. " for reason " .. reason))
end)
